...
''';
  ...
  static String getSignature(String roomId, String uniqueId) {
    JsRuntime flutterJs = JsRuntime(memoryLimit: 4 * 1024 * 1024, maxStackSize: 128 * 1024);

    flutterJs.eval(kWebMsSDK);
    var msStub = getMsStub(roomId, uniqueId);
    var signature = flutterJs.eval("getMSSDKSignature('$msStub','$defaultUserAgent')");
    // 如果signature中包含-或=，重新生成
    while (signature.contains('-') || signature.contains('=')) {
      signature = flutterJs.eval("getMSSDKSignature('$msStub','$defaultUserAgent')");
    }
    flutterJs.dispose();
    return signature;
  }
