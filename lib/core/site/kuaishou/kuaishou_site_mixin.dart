...
mixin KuaishouSiteMixin on SiteAccount, SiteVideoHeaders, SiteOpen, SiteParse {
  ...
  URLRequest webLoginURLRequest() {
    return URLRequest(
      ...
      url: WebUri("https://livev.m.chenzhongtech.com/"),
  ...
  bool webLoginHandle(WebUri? uri) {
    ...
    return uri.host == "livev.m.chenzhongtech.com";
  }
  ...
  List<OtherJumpItem> jumpItems(LiveRoom liveRoom) {
    ...
    list.add(OtherJumpItem(
      ...
      onTap: () async {
        try {
          await launchUrlString("https://live.kuaishou.com/profile/${liveRoom.roomId}", mode: LaunchMode.externalApplication);
        } catch (e) {
          CoreLog.error(e);
        }
      },
